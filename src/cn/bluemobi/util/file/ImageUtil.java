...
public class ImageUtil {
	...
	public static void scaleImage(String oldpath, String newpath, int wdith,
		...
		//文件格式
		int index = oldpath.lastIndexOf(".");
		String formatName = oldpath.substring(index+1);
	 ...
	 *            缩略后的高
	 */
	public static void scaleImageInRatio(String oldpath, String newpath, int maxWdith,
			int maxHeight) throws IOException, Exception {
		// 获取老的图片
		File oldimg = new File(oldpath);
		BufferedOutputStream out = null;
		//文件格式
		...
		try {
			...
			double width = 0;
			double height = 0;
			if(((double)bi.getWidth() / maxWdith) > ((double)bi.getHeight()/maxHeight)){
				width = (double)bi.getWidth() > maxWdith ? maxWdith : (double)bi.getWidth();
				height = (double)bi.getHeight()/((double)bi.getWidth() / width);
			}else{
				height = (double)bi.getHeight() > maxHeight ? maxHeight : (double)bi.getHeight();
				width = (double)bi.getWidth()/((double)bi.getHeight()/height);
			}
			
			Image temp = bi.getScaledInstance((int)width, (int)height,
					BufferedImage.SCALE_SMOOTH);
			BufferedImage thumbnail = new BufferedImage((int)width, (int)height,
					BufferedImage.TYPE_INT_RGB);
			thumbnail.getGraphics().drawImage(temp, 0, 0, null);
