...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="admin/advertisement/list.htm?type=3" method="post" name="userForm" id="userForm">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						<th class="center" style="width:5%" onclick="selectAll()">
					...
					<c:when test="${not empty advertisementList}">
						<c:forEach items="${advertisementList}" var="var" varStatus="vs">
							<tr>
								...
								<td  class="center linkUrl" style="vertical-align:middle;">${var.linkUrl}</td>
								...
								<td class="center" style="vertical-align:middle;">
									<div class='hidden-phone visible-desktop btn-group'>
											<a <c:if test="${vs.index == 0}"> class='btn btn-mini disabled' title="升序" </c:if> <c:if test="${vs.index!=0}"> class='btn btn-mini  btn-info' title="升序"  onclick="upOrDown('${var.id }','${var.number }','5');" </c:if>><i class='icon-arrow-up'></i></a>
											<a <c:if test="${vs.index == 4}"> class='btn btn-mini disabled' title="降序" </c:if> <c:if test="${vs.index!=4}"> class='btn btn-mini  btn-info' title="降序"  onclick="upOrDown('${var.id }','${var.number }','4');" </c:if> ><i class='icon-arrow-down'></i></a>
									</div>
								</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function editUser(id){
			//编辑页面
			top.mainFrame.tabAddHandler("advertisementStarList","剧本工厂轮播图资料","admin/advertisement/goEditAdvertisement.htm?id="+id);
		}
