...
<body>
		...
		<div id="loginbox">
			<form action="" method="post" name="loginForm"
				...
				<div class="control-group normal_text">
					<h3 style="text-align: left; padding-left: 36">
						<img src="static/login/yougushi.png" style="height:50px;" alt="Logo" />
					</h3>
				</div>
					...
					<div style="width:86%;padding-left:8%;">
						...
						<div style="float: left;" class="codediv">
							<input type="text" name="code" id="code" class="login_code"
								style="height:20px; width:60px;  padding-top:3px;" />
						</div>
						<div style="float: left;">
			...
			<div class="controls">
				<div class="main_input_box">
					<font color="white"><span id="nameerr">Copyright © BlueMobi
							2100</span></font>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		//服务器校验
		function severCheck(){
			$("#to-recover").removeAttr("onclick");//去掉a标签中的onclick事件
			if(check()){
				...
				$.ajax({
					type: "POST",
					url: $("base")[0].href+"admin/login/submit.htm?uuid="+Math.random(),
			    	data: {username:username,password:password,code:code},
					dataType:'json',
					cache: false,
					success: function(data){
						if( data.status==1){
							window.location.href= $("base")[0].href+"admin/index.htm";
						}else{
							$("#to-recover").attr("onclick","severCheck();");
							 if("usernameError" == data.info){
								$("#loginname").tips({
									side : 1,
									msg : "用户名有误",
									bg : '#FF5080',
								...
								$("#loginname").focus();
							}else if("passwordError" == data.info){
								$("#password").tips({
									side : 1,
									msg : "密码错误",
									bg : '#FF5080',
