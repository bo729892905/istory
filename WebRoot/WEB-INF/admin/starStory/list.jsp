...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="admin/starStory/list.htm" method="post" name="userForm" id="userForm">
			<table border='0' >
				<tr>
				
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="title" value="${starStory.title }" placeholder="文章标题" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td style="padding-left: 10px">
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="author" value="${starStory.author }" placeholder="发布人" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td style="padding-left: 10px"><input class="span10" name="releaseTime" id="releaseTime" value="${starStory.releaseTime}" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true})" readonly="readonly" style="width:88px;" placeholder="发布日期"/></td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center" style="width:15%">期数</th>
						<th class="center" style="width:15%">浏览次数</th>
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty starStoryList }">
						<c:forEach items="${starStoryList}" var="star" varStatus="vs">
							<tr>
								<td class="center" style=" padding-bottom: 5px"><label><input type='checkbox' name='ids' value="${star.id }" /><span class="lbl"></span></label></td>
								<td class="center">${star.id }</td>
								<td class="center">${star.title }</td>
								<td class="center">${star.author }</td>
								<td class="center">${star.number }</td>
								<td class="center">${star.viewCount }</td>
								<td class="center">${star.releaseTime }</td>
								<td class="center">${star.hot }</td>
								<td style="width: 5%;" class="center">
									<c:if test="${star.status == '0' }"><span class="label label-important arrowed-in">禁用</span></c:if>
									<c:if test="${star.status == '1' }"><span class="label label-success arrowed">开启</span></c:if>
								</td>
								<td  style="width: 4%; padding-left: 20px"  >
									<div class='hidden-phone visible-desktop btn-group'>
										<a class='btn btn-mini btn-info' title="查看" onclick="editUser('${star.id }');"><i class='icon-edit'></i></a>
									</div>
								</td>
							</tr>
		...
		<table style="width:100%;">
			<tr>
				...
				<td style="vertical-align:top;">
					<a class="btn btn-small btn-primary" onclick="toAdd();">新增</a>
				</td>
				<td style="vertical-align:top;">
