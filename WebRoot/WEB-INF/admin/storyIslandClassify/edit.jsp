...
<div class="container-fluid" id="main-container">
	<div class="row-fluid">
		...
		<div id="div_content" class="row-fluid" style="padding-top: 100px;">
			<form class="form-horizontal" id="validation-form" method="post">
					<div class="control-group">
						<label class="control-label" for="title">原分类名:</label>
						<div class="controls" >
							<div class="span12">
								<input type="hidden" name="id" id="id" value="${storyIslandClassify.id }" >
								<input type="text" readonly="readonly" name="oldClassify" id="oldClassify" value="${storyIslandClassify.classify }" >
							</div>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for="author">新分类名:</label>
						<div class="controls">
							<span class="span12"><input type="text"  name="classify" id="classify" value="" maxlength="32" > 
							</span>
						</div>
					</div>
		...
		<div class="form-actions" style="text-align: center; margin-top: 160px">
...
$(document).ready(function(){
	$("#submit").click(function(){
		...
		if(vailVals()!=0){
		 ...
		 bootbox.confirm("你将修改故事分类名！","取消","确定",function(e){
			if(e){
				var data = {
					"id":$("#id").val(),
					"classify":$("#classify").val(),
				};
				$.post("admin/storyIslandClassify/save.htm", data, function(request) {
					var json = eval('(' + request + ')'); 
					if(1 == json.status){
						$("#div_content").hide();
						top.Dialog.close();
						targetMainFrame("admin/storyIslandClassify/list.htm");
					}else{
...
//验证
function vailVals(){
	if(!checkField($("#classify").val(), 1, 32)){
			valTips('classify', '请输入正确的分类名');
			return 1;
	}
	return 0;
}
