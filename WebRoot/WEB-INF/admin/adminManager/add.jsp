		...
		<div id="div_content" class="row-fluid" style="padding-top: 10px;">
			<form class="form-horizontal" id="validation-form" method="post">
					...
					<div class="control-group">
						...
						<div class="controls">
							<span class="span12"> <input  type="text" id="name" name="name" placeholder="请输入姓名" />
							</span>
						</div>
					</div>
...
$(document).ready(function(){
	$("#submit").click(function(){
		...
		if(vailVals()!=0){
		 ...
		 bootbox.confirm("你将新增管理员！","取消","确定",function(e){
			if(e){
				var data = {
				...
				//action="user/addUser"
				$.post("admin/add.htm", data, function(request) {
					...
					if(1 == json.status){
						...
						if("name_exist"==json.msg){
							valTips('username', '该账号已存在!');
						}else{
...
function vailVals(){
	...
	if($("#password").val() != ''){
		if($("#password").val() != $("#password1").val()){
			...
			return 1;
		}else if(!checkField($("#password").val(), 5, 20,new RegExp("^[a-zA-Z0-9]{5,20}$"))){
