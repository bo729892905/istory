		...
		<div id="div_content" class="row-fluid" style="padding-top: 10px;">
			<form class="form-horizontal" id="validation-form" method="post">
					<div class="control-group">
						...
						<div class="controls" >
							<div class="span12">
								<input type="hidden" name="id" id="id" value="${adminU.id }" />
								<input type="text"  name="name" id="name" value="${adminU.name }"  />
							</div>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for="username">原密码:</label>
						<div class="controls" >
							<div class="span12">
					...
					<div class="control-group">	
						<label class="control-label" for="username">确认新密码:</label>
						<div class="controls" >
							<div class="span12">
...
$(document).ready(function(){
	$("#submit").click(function(){
		...
		if(vailVals()!=0){
		 ...
		 bootbox.confirm("你将修改个人资料！","取消","确定",function(e){
			if(e){
				var data = {
					"id":$("#id").val(),
					"name":$("#name").val(),
...
//验证
function vailVals(){
	if(!checkField($("#name").val(), 1, 32)){
			valTips('name', '请输入正确的名称');
			return 1;
	}
	var password1 = $("#password1").val();
	var password2 = $("#password2").val();
	var password = $("#password").val();
	if((password1 !=''&& password1 != undefined) || (password2 !=''&& password2 != undefined) || (password !=''&& password != undefined)){
		if($("#password1").val() == ''){
			valTips('password1', '如要修改密码，请输入原密码');
			return 1;
		}
		if($("#password2").val() == ''){
			valTips('password2', '请输入新密码');
			return 1;
		}
		if($("#password").val() == ''){
			valTips('password', '请再次输入新密码');
			return 1;
		}
		if($("#password2").val() != $("#password").val()){
			valTips('password', '两次密码不一致');
			return 1;
		}else if(!checkField($("#password").val(), 5, 20)){
			valTips('password', '请输入5到20位字母或数字组合');
			return 1;
		}
	}
	return 0;
